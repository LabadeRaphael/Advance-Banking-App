...
const signUp=()=>{
    ...
    if(checkName == "true" && checkNum == "true" && checkEmail == "true" && checkPassword == "true" && checkConfirmPassword == "true" && checkPinNum == "true") {
        allUserArray.push(eachUserDetails)
        console.log(allUserArray);
        document.getElementById("signUpSuccessful").innerHTML=`<div class='alert alert-info col-lg-6 mx-auto d-block text-center fw-bold'> Sign Up Successful</div>`
        localStorage.setItem("allUser",JSON.stringify(allUserArray))
        window.location="Sign In Page.html"
    }else if (Name.value == "" && PhoneNum.value == "" && Email.value == "" && Password.value =="" && ConfirmPassword.value == "" && pinNumber.value == "") {
        ...
        if(Name.value == ""){
            document.getElementById("nameMsg").innerText="Fill this field  correctly"
        }
        if(PhoneNum.value == ""){
            document.getElementById("phoneMsg").innerText="Fill this field correctly"
        }
        if(Email.value == ""){
            document.getElementById("emailMsg").innerText="Fill this field correctly"
        }
        if(Password.value  == ""){
            document.getElementById("passwordMsg").innerText="Fill this field correctly"
        }
        if(ConfirmPassword.value  == ""){
            document.getElementById("confirmPasswordMsg").innerText="Fill this field correctly"
        }
        if(pinNumber.value  == ""){
...
const validateEmail=()=>{
    ...
    var validRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z]+(?:\.[a-zA-Z0-9-]+)*$/;
...
const validatePasword=()=>{
    ...
    document.getElementById("passwordMsg").innerText="Password must be at least 8 characters"
    localStorage.setItem("isPasswordCorrect",false)
    } else if(password.search(/[a-z]/) < 0) {
        document.getElementById("passwordMsg").innerText="Password must contain at least one lowercase letter"
        localStorage.setItem("isPasswordCorrect",false)
    } else if(password.search(/[A-Z]/) < 0) { 
        document.getElementById("passwordMsg").innerText="Password must contain at least one uppercase letter"
        localStorage.setItem("isPasswordCorrect",false)
        } else if(password.search(/[0-9]/) < 0) { 
        document.getElementById("passwordMsg").innerText="Password must contain at least one number"
        localStorage.setItem("isPasswordCorrect",false)
    } else { 
